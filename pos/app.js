...
async function loadSystemData(silent = false) {
    ...
    try {
        ...
        db.categorias = data.categorias.sort((a, b) => parseInt(a.Orden) - parseInt(b.Orden));
        ...
        db.productos = data.productos
            ...
            .map(p => ({
                ...
                Color: mapColor(p['Color Boton'] || 'primary'),
...
function attemptLogin() {
    ...
    if (usuario) {
        ...
        if (displayEl) {
            ...
            console.warn("Advertencia: No existe el elemento con id='user-display' en el HTML.");
        }
        ...
        document.getElementById('login-error').innerText = 'PIN incorrecto o usuario inactivo';
        pinInput.value = '';
        pinInput.focus();
    }
}
...
function updateClock() {
    ...
    if(el) {
        const now = new Date();
        el.innerText = `${now.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} | T${currentTurnData.idTurno}`;
    }
}
...
function renderProducts(lista) {
    ...
    container.innerHTML = lista.map(p => `
        <div class="col-4 col-md-3">
            <div class="card product-card h-100 text-white bg-${p.Color} shadow-sm border-0" onclick="addToCart('${p.ID_Producto}')">
                <div class="card-body d-flex flex-column align-items-center justify-content-center text-center p-2">
                    <h6 class="card-title fw-bold mb-1" style="font-size: 0.9rem;">${p.Nombre}</h6>
                    <span class="badge bg-white text-dark bg-opacity-75 mt-auto">${formatter.format(p.Precio)}</span>
                </div>
            </div>
        </div>
    `).join('');
...
async function saveToDatabase(payload) {
    try {
        ...
        alert("¡ATENCIÓN! La venta se hizo pero no se guardó en la nube (Error de red). Anótala manualmente.");
